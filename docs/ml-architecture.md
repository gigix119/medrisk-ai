...
## Why no FastAPI integration yet

Phase 2 produces a portable, self-verifying model bundle (`medrisk_ml/registry/bundle.py`) precisely so that Phase 3 can load it without depending on any Phase-2-only code (training loops, dataset classes, the CLI). Integrating inference into `app/` now would couple the API to in-progress ML code before the model, threshold, and calibration are actually finalized — see [experiment-protocol.md](experiment-protocol.md) for why those are frozen in a specific order.
